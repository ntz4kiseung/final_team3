...
-- 개설모임 조회 리스트
SELECT P.USERID AS TAKEUSERID, P.POSTID, RV.GIVEUSERID,PT2.URL AS GIVEUSERURL, RV.GRADE, RS.BADGEPOINTID, PT.URL AS GIVEBADGEURL, RS.CONTENTS 
FROM POST P LEFT JOIN REVIEW RV ON (P.USERID=RV.TAKEUSERID AND P.POSTID=RV.POSTID)
            JOIN REVIEW_SUB RS ON RV.REVIEWID=RS.REVIEWID
            JOIN POINT PT ON RS.BADGEPOINTID=PT.BADGEPOINTID
            JOIN USER_ESSENTIAL UE ON RV.GIVEUSERID=UE.USERID
            JOIN USER_BADGE_LOG UBL ON UE.BADGELOGID=UBL.BADGELOGID
            JOIN POINT PT2 ON UBL.BADGEPOINTID=PT2.BADGEPOINTID
WHERE P.POSTID='PT01285'
;


SELECT J.USERID AS TAKEUSERID, J.POSTID, J.JOINID, RV.GIVEUSERID,PT2.URL AS GIVEUSERURL, RV.GRADE, RS.BADGEPOINTID, PT.URL AS GIVEBADGEURL, RS.CONTENTS 
FROM JOIN J LEFT JOIN REVIEW RV ON J.POSTID=RV.POSTID
            LEFT JOIN REVIEW_SUB RS ON RV.REVIEWID=RS.REVIEWID
            LEFT JOIN POINT PT ON RS.BADGEPOINTID=PT.BADGEPOINTID
            LEFT JOIN USER_ESSENTIAL UE ON RV.GIVEUSERID=UE.USERID
            LEFT JOIN USER_BADGE_LOG UBL ON UE.BADGELOGID=UBL.BADGELOGID
            LEFT JOIN POINT PT2 ON UBL.BADGEPOINTID=PT2.BADGEPOINTID
WHERE J.POSTID='PT01031'
  AND J.STATUSID='ST00003'
  AND J.USERID='BROGAN'
;

SELECT T1.TAKEUSERID, T2.GIVEUSERID, UE.NICKNAME, PT.URL
    , (SELECT GRADE FROM REVIEW WHERE GIVEUSERID=T2.GIVEUSERID AND TAKEUSERID=T1.TAKEUSERID AND POSTID='PT01031') AS GRADE
FROM
(
...
-- FN_GET_REVIEW_GRADE
CREATE OR REPLACE FN_GET_REVIEW_GRADE(V_GIVEUSERID VARCHAR, V_TAKEUSERID VARCHAR, )
