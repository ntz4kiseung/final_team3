...
SELECT USERID, URL, TITLE, NICKNAME, MEETDATE, ADDRSINAME, ADDRGUNAME, POSTDATE, ADDRDETAIL, INTERMAINNAME, INTERSUBNAME, INTERDETAIL, DRINK, MOODNAME, SAMEGENDER, CONTENTS, MINNUM, MAXNUM, LIMITGRADE
,NVL((SELECT C.CERTINAME FROM USER_CERTIFICATION U JOIN CERTIFICATION_CATE C
ON U.CERTIID = C.CERTIID WHERE U.USERID= POST_VIEW.USERID AND C.CERTIID = 'CT00001'), 'v') TELCERTIID
, NVL((SELECT C.CERTINAME FROM USER_CERTIFICATION U JOIN CERTIFICATION_CATE C
ON U.CERTIID = C.CERTIID WHERE U.USERID= POST_VIEW.USERID AND C.CERTIID = 'CT00002'), 'v') EMAILCERTIID
, NVL((SELECT ID FROM FOLLOW WHERE GIVEUSERID = 'anlant' AND TAKEUSERID = POST_VIEW.USERID), 0) FOLLOWID
FROM POST_VIEW
WHERE POSTID = 'PT00002';
...
, (SELECT C.CERTINAME FROM USER_CERTIFICATION U JOIN CERTIFICATION_CATE C
ON U.CERTIID = C.CERTIID WHERE C.CERTIID = 'CT00001') TELCERTIID
, (SELECT C.CERTINAME FROM USER_CERTIFICATION U JOIN CERTIFICATION_CATE C
ON U.CERTIID = C.CERTIID WHERE C.CERTIID = 'CT00002') EMAILCERTIID
FROM 
(
SELECT C.CERTINAME CERTINAME, C.CERTIID CERTIID, U.USERID USERID
...
-- 휴대폰 인증 여부 뷰
CREATE OR REPLACE VIEW TEL_CERTIFICATION_CATE_VIEW
AS
SELECT U.USERID USERID, C.CERTINAME CERTINAME
FROM USER_CERTIFICATION U LEFT JOIN CERTIFICATION_CATE C
ON U.CERTIID = C.CERTIID 
WHERE U.CERTIID = 'CT00001';

-- 메일 인증 여뷰 뷰
CREATE OR REPLACE VIEW MAIL_CERTIFICATION_CATE_VIEW
AS
SELECT U.USERID USERID, C.CERTINAME CERTINAME
FROM USER_CERTIFICATION U LEFT JOIN CERTIFICATION_CATE C
ON U.CERTIID = C.CERTIID 
WHERE U.CERTIID = 'CT00002';
...
-- 댓글 삽입
INSERT INTO JOIN(JOINID, POSTID, USERID, STATUSID, CONTENTS, JOINDATE) VALUES(ADD_ID('JOIN','JOINID','JN'),'PT00002','anlant','안녕하세요',SYSDATE);
